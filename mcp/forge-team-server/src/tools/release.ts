...
export function registerReleaseTools(server: McpServer, db: ForgeDatabase) {
  ...
  server.registerTool("release.mark_ready", { description: "Mark release ready if gates are clear.", inputSchema: {} }, async () => { const ready = verifyReady(db); if (ready.ready) db.run("UPDATE project_state SET value='RELEASE_READY' WHERE key='governor.state'"); return jsonResult(ready); });
}
