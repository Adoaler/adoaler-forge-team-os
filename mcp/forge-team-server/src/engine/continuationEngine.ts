...
export function saveContinuation(db: ForgeDatabase, input: { file_path: string; current_hash: string; last_completed_symbol?: string; open_blocks?: string[]; imports_declared?: string[]; public_api_emitted?: string[]; remaining_plan: string; critic_warnings?: string[]; tests_required?: string[] }) {
...
export function nextContinuation(db: ForgeDatabase, filePath?: string) {
  if (filePath) return db.get("SELECT * FROM continuations WHERE file_path = ? AND status = 'partial' ORDER BY updated_at DESC LIMIT 1", [filePath]) ?? null;
  return db.get("SELECT * FROM continuations WHERE status = 'partial' ORDER BY updated_at DESC LIMIT 1") ?? null;
}

export function markComplete(db: ForgeDatabase, filePath: string) {
  const now = nowIso();
...
export function resumePrompt(db: ForgeDatabase, filePath: string) {
