...
export function getFileState(db: ForgeDatabase, filePath: string) {
  return db.get("SELECT * FROM files WHERE file_path = ?", [filePath]) ?? null;
}

export function updateFileState(db: ForgeDatabase, input: { file_path: string; status?: string; current_hash?: string; last_completed_symbol?: string; open_blocks?: string[]; imports_declared?: string[]; public_api_emitted?: string[]; remaining_plan?: string; critic_warnings?: string[]; tests_required?: string[]; last_chunk_id?: string }) {
  const now = nowIso();
