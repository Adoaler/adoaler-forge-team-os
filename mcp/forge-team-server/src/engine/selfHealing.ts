...
export function classifyFailure(text: string) {
  if (/SyntaxError|parse/i.test(text)) return "syntax_error";
  if (/TS\d+|type/i.test(text)) return "type_error";
  if (/test failed|expect\(/i.test(text)) return "test_failure";
  if (/circular/i.test(text)) return "circular_dependency";
  if (/secret|token|private key/i.test(text)) return "security_risk";
  if (/continuation|hash mismatch/i.test(text)) return "continuation_corruption";
  return "integration_failure";
}

export function planRepair(db: ForgeDatabase, input: { failure: string; affected_files?: string[] }) {
...
export function closeRepair(db: ForgeDatabase, input: { title: string; category: string; symptoms: string; fix_summary: string; affected_files?: string[] }) {
  const now = nowIso();
  const bugId = id("bug");
