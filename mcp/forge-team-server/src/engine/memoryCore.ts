...
export function writeMemory(db: ForgeDatabase, input: { project_id?: string; kind: string; topic: string; content: string; reason?: string; task_id?: string }) {
...
export function searchMemory(db: ForgeDatabase, query: string, limit = 20) {
  const like = "%" + query + "%";
