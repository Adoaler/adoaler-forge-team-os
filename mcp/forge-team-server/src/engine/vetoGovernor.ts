...
export function getGovernorState(db: ForgeDatabase) {
  return db.get<{ value: string }>("SELECT value FROM project_state WHERE key = 'governor.state'")?.value ?? "NORMAL";
}

export function setGovernorState(db: ForgeDatabase, state: string) {
...
export function governorCheck(db: ForgeDatabase) {
  ...
  const vetoes: string[] = [];
  if (latestReject && state !== "REPAIR_ONLY") vetoes.push("Critic rejected but governor is not in REPAIR_ONLY");
  if (partialWithoutContinuation.length) vetoes.push("Partial files without continuation records");
  if (openChanges.length && !["CHANGE_REQUEST", "IMPACT_ANALYSIS", "REPLAN", "MIGRATION"].includes(state)) vetoes.push("Open Change Request outside change flow");
