...
export function validateChunk(input: { file_path: string; content: string }) {
  ...
  for (const stmt of source.statements) {
    ...
    if ((ts.isFunctionDeclaration(stmt) || ts.isClassDeclaration(stmt) || ts.isInterfaceDeclaration(stmt) || ts.isTypeAliasDeclaration(stmt)) && stmt.name) key = ts.SyntaxKind[stmt.kind] + ":" + stmt.name.text;
...
export function findSafeBoundary(content: string) {
  const trimmed = content.trimEnd();
  if (trimmed.length === 0) return { offset: 0, kind: "empty" };
  const lastNewline = trimmed.lastIndexOf("\n");
  const lastLine = trimmed.slice(lastNewline + 1);
  const openBraces = (trimmed.match(/\{/g) ?? []).length - (trimmed.match(/\}/g) ?? []).length;
  if (openBraces === 0 && /[;}\]]$/.test(trimmed)) return { offset: trimmed.length, kind: "syntax_valid_boundary" };
  if (/^(export\s+)?(async\s+)?function\s+|^(export\s+)?class\s+|^describe\(/.test(lastLine)) return { offset: lastNewline, kind: "before_incomplete_symbol" };
