...
export function checkpoint(db: ForgeDatabase, label: string) {
  const files = db.all<{ file_path: string; current_hash: string }>("SELECT file_path, current_hash FROM files ORDER BY file_path");
...
export function verifyReady(db: ForgeDatabase) {
  ...
  const blockers = [];
  if (partial.length) blockers.push("partial files exist");
  if (rejected) blockers.push("latest critic rejected");
  if (openChanges.length) blockers.push("open change requests exist");
  if (incomplete.length) blockers.push("task graph has incomplete work");
