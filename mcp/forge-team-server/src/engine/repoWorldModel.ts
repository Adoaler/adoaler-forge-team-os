...
function listFiles(dir: string, out: string[] = []): string[] {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (entry.isFile() && textExt.has(extname(entry.name).toLowerCase())) out.push(full);
  }
  return out;
}

export function extractSymbols(filePath: string, text: string) {
  const ext = extname(filePath).toLowerCase();
  if (![".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs"].includes(ext)) return [];
  const source = ts.createSourceFile(filePath, text, ts.ScriptTarget.Latest, true);
  const result: Array<{ name: string; kind: string; exported: boolean; signature?: string }> = [];
...
export function extractImports(filePath: string, text: string): string[] {
  ...
  for (const stmt of source.statements) {
    if (ts.isImportDeclaration(stmt) && ts.isStringLiteral(stmt.moduleSpecifier)) imports.push(stmt.moduleSpecifier.text);
    if (ts.isExportDeclaration(stmt) && stmt.moduleSpecifier && ts.isStringLiteral(stmt.moduleSpecifier)) imports.push(stmt.moduleSpecifier.text);
  }
  return imports;
}

export function indexRepo(db: ForgeDatabase, root = workspaceRoot()) {
  const now = nowIso();
...
export function searchText(db: ForgeDatabase, query: string, limit = 20) {
  const root = workspaceRoot();
  const hits: Array<{ file_path: string; line: number; preview: string }> = [];
  for (const row of db.all<{ file_path: string }>("SELECT file_path FROM files ORDER BY file_path")) {
    const full = resolve(root, row.file_path);
    if (!existsSync(full) || statSync(full).size > 1024 * 1024) continue;
    const lines = readFileSync(full, "utf8").split("\n");
    for (let i = 0; i < lines.length; i++) {
      if (lines[i]?.toLowerCase().includes(query.toLowerCase())) hits.push({ file_path: row.file_path, line: i + 1, preview: (lines[i] ?? "").trim().slice(0, 240) });
...
export function detectDuplicateSymbols(db: ForgeDatabase) {
...
export function detectCycles(db: ForgeDatabase) {
  const rows = db.all<{ from_file: string; to_file: string }>("SELECT from_file, to_file FROM dependencies WHERE from_file IS NOT NULL AND to_file IS NOT NULL");
  const edges = new Map<string, string[]>();
  for (const row of rows) {
    if (!row.to_file.startsWith(".")) continue;
