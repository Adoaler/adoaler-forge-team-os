...
export function listReadyTasks(db) {
    const tasks = db.all("SELECT * FROM tasks WHERE status IN ('planned','ready') ORDER BY created_at");
    const complete = new Set(db.all("SELECT id FROM tasks WHERE status = 'complete'").map((row) => row.id));
...
export function setTaskStatus(db, taskId, status, reason) {
    const now = nowIso();
