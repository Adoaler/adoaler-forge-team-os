...
export function review(db, input) {
    const blocking = [];
    const notes = [];
    const text = input.content ?? "";
    if (/T[O]DO\b/.test(text))
        blocking.push("deferred-work marker in primary content");
    if (/placeholder|stub|fake implementation/i.test(text))
        blocking.push("Misleading placeholder or fake implementation wording");
    if (/catch\s*\([^)]*\)\s*\{\s*\}/.test(text))
        blocking.push("Swallowed error block");
    if (/process\.env\.[A-Z0-9_]*(TOKEN|SECRET|KEY|PASSWORD)/.test(text))
        notes.push("Reads sensitive environment variable; verify it is not logged or persisted");
...
export function latestReport(db) {
