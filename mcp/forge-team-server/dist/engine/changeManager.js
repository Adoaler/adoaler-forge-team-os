...
export function impactAnalysis(db, changeId) {
    const change = db.get("SELECT * FROM change_requests WHERE id = ?", [changeId]);
    if (!change)
    ...
    for (const word of words) {
        for (const row of db.all("SELECT file_path FROM files WHERE file_path LIKE ? LIMIT 20", ["%" + word + "%"]))
            affectedFiles.add(row.file_path);
    }
...
export function createMigrationTasks(db, changeId) {
    const change = db.get("SELECT * FROM change_requests WHERE id = ?", [changeId]);
    if (!change)
        throw new Error("Change Request not found: " + changeId);
    const task = createTask(db, {
        ...
        type: "migration",
        allowed_files: JSON.parse(change.affected_files || "[]"),
